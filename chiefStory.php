...
<div class="container departments">
    <input class="form-control mb-4 admTable" id="tableSearch" type="text" placeholder="Введите имя отправителя или получателя">
  <table>
    <thead>
      <tr>
        <th scope="col">ID</th>
...
<?php
  ...
  FROM ideas
  LEFT JOIN user on ideas.sender = user.id AND user.department = $sesDep
  WHERE sender = user.id AND (status = 2 OR status = 4 OR status = 5)";
  ...
  $rats = mysqli_query($connection, "SELECT
     ...
     chiefs.family as 'cFamily', chiefs.name as 'cName', chiefs.father as 'cFather',
